...
class TestApi:
    ...
    _id_step_5 = 5
    _expected_userId = 10
    _json_path = os.path.join('..', 'test_data', 'user_data.json')

    @pytest.mark.test_api
    def test_api(self):
        Logger.info("Шаг 1. Отправьте запрос GET, (/posts).")
        responce_from_url = ApiUtils.get(ConfigManager.get_config_value("Url"), ConfigManager.get_config_value("Posts"))
        assert responce_from_url.status_code == requests.codes.ok, "Статус код не равен 200"
        assert ApiUtils.is_content_type_json(responce_from_url), "Тело ответа не JSON"
        ...
        responce_from_url = ApiUtils.get(ConfigManager.get_config_value("Url"),
                                         ConfigManager.get_config_value("ParamPost").format(str(self._id_step_2)))
        assert responce_from_url.status_code == requests.codes.ok, "Статус код не равен 200"
        ...
        responce_from_url = ApiUtils.get(ConfigManager.get_config_value("Url"),
                                         ConfigManager.get_config_value("ParamPost").format(str(self._id_step_3)))
        ...
        responce_from_url = ApiUtils.post(ConfigManager.get_config_value("Url"),
                                          ConfigManager.get_config_value("Posts"), post_data)
        ...
        responce_from_url = ApiUtils.get(ConfigManager.get_config_value("Url"),
                                         ConfigManager.get_config_value("Users"))
        assert ApiUtils.is_content_type_json(responce_from_url), "Тело ответа не JSON"
        assert responce_from_url.status_code == requests.codes.ok, "Статус код не равен 200"
        value_from_responce = ApiUtils.get_value(responce_from_url, self._id_step_5,
                                                 ConfigManager.get_config_value("Value"))
        assert (value_from_responce == ApiUtils.get_expected_data(User, self._json_path)), \
            "Данные пользователя не соответствуют ожидаемым"

        Logger.info("Шаг 6. Отправьте запрос GET,  с id=5 (/users/5).")
        responce_from_url = ApiUtils.get(ConfigManager.get_config_value("Url"),
                                         ConfigManager.get_config_value("ParamUsers").format(str(self._id_step_5)))
        assert responce_from_url.status_code == requests.codes.ok, "Статус код не равен 200"
        assert User.from_dict(json.loads(responce_from_url.text)) == value_from_responce, \
            "Данные пользователя не соответствуют ожидаемым"
